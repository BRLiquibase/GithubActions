...
violations = []
for stmt in re.findall(r"\b(create\s+table|add\s+column)\b[\s\S]*?;", sql, flags=re.IGNORECASE):
    if re.search(r"\bvarchar(?!\s*\()", stmt):
        violations.append("unbounded varchar")
    if re.search(r"\btext\b", stmt):
        violations.append("text type")
if violations:
    end_fail("Disallowed types detected: " + ", ".join(sorted(set(violations))))
else:
    end_ok("No unbounded VARCHAR/TEXT types detected.")
