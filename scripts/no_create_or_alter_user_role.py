...
patterns = [r"\bcreate\s+user\b", r"\balter\s+user\b", r"\bdrop\s+user\b", r"\bcreate\s+role\b.*\blogin\b", r"\balter\s+role\b", r"\bdrop\s+role\b"]
for p in patterns:
    if re.search(p, sql):
        end_fail("User/role management DCL detected in changeset. Not allowed.")
end_ok("No user/role DCL detected.")
