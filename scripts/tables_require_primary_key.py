...
violations = []
for m in re.finditer(r"\bcreate\s+table\b[\s\S]*?;", sql_all):
    stmt = m.group(0)
    if "primary key" not in stmt and not re.search(r"\btemporary\b|\btemp\b", stmt):
        snippet = stmt.strip().replace("\n"," ")
        if len(snippet)>200: snippet = snippet[:200]+"..."
        violations.append(snippet)
if violations:
    end_fail("Table(s) without PRIMARY KEY: " + "; ".join(violations[:3]) + ("" if len(violations)<=3 else f" (+{len(violations)-3} more)"))
else:
    end_ok("All created tables declare a PRIMARY KEY.")
