...
pan_like = ["pan","card_number","credit_card","cc_number","ccnum","cardnum"]
ok_tokens = ["token","hash","hashed","digest","masked","mask","enc","encrypted"]
violations = []
...
for stmt in pattern.findall(sql_all):
    for kw in pan_like:
        if re.search(rf"\b{re.escape(kw)}\b", stmt):
            col_fragment = stmt
            if not any(tok in col_fragment for tok in ok_tokens):
...
if violations:
    ...
    end_fail(f"PCI DSS: PAN-like column without token/hash/mask indicator: {details}{more}")
else:
    end_ok("No obvious raw PAN-like columns.")
