...
if re.search(r"\bdisable\s+trigger\b", sql) or re.search(r"\balter\s+table\b[\s\S]*?\bdisable\s+trigger\b", sql):
    end_fail("SOX: Disabling triggers detected in changeset.")
end_ok("No trigger disabling commands detected.")
